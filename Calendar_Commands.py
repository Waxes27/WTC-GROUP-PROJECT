...
def calendar_data(service):
    cal_data = service.calendars().get(calendarId='test').execute()
...
#deleting a event     
def deleting_event(service):
    del_event = service.events.delete(calendarId='primary',eventId='eventId',sendNotifications=True,sendUpdates=True)
